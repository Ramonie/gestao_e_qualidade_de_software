...
class MockBD(TestCase):
    ...
    try:
            con = conectar(BD)
            cursor = con.cursor()
            cursor.execute('CREATE TABLE aluno (id int NOT NULL PRIMARY KEY,nome text NOT NULL)')

            cursor.execute('CREATE TABLE Disciplina (id int NOT NULL PRIMARY KEY, nome text NOT NULL)')   
